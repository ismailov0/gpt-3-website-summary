"""
Модуль содержит класс WebSiteAssistant, который позволяет создавать ассистента,
способного изучать текст веб-страниц с целью составить краткое резюме
из основных идей на русском языке.

Класс содержит следующие методы:

    - __init__(self, url: str) -> None: конструктор класса Assistant, 
        принимающий URL-адрес веб-страницы.
    - summary(self) -> str: метод для создания краткого резюме из основных идей веб-страницы.
"""
import sys
import os
import openai
...
class WebSiteAssistant:
    """
    Класс для создания ассистента, который изучает текст с целью \
        составить краткое резюме из его основных идей на русском языке.
    """
    ...
    def _send_message_to_openai(self, cleaned_html_text: str) -> str:
        ...
        messages = [
            {
                "role": "system", "content": "Ты - асистент, который изучает текст с целью' \
                      ' составить краткое резюме из его основных идей на русском языке"
            },
        ]
